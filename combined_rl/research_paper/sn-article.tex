...
\node[box, fill=blue!20] (gen) at (0, 8) {Message Generation\\UAVs + GCS};

% Messages flow
\node[label] (msg1) at (0, 7) {Telemetry, Commands,\\Navigation, Payload};

% Layer 2: ML-Based Prioritization
\node[box, fill=green!20] (ml) at (0, 5.5) {ML Prioritizer\\(Random Forest)};
\node[label, below=0.2cm of ml] {Feature Extraction:\\msg\_type, payload, freq, SNR, mission\_phase};

% Single priority queue
\node[box, fill=yellow!20] (q) at (0, 3.5) {Priority Queue\\(Priority-aware)};

% Draw arrow from ML to queue
\draw[arrow] (ml) -- (q);

% Layer 3: State Discretization
\node[box, fill=purple!20] (state) at (0, 1.5) {State Discretization\\$\mathbf{s}_t = (q_{\text{len}}, u_{\text{frac}}, d_{\text{urgency}}, c_{\text{quality}})$};

% Draw arrow from queue to state
\draw[arrow] (q) -- (state);

% Layer 4: RL Agent
\node[box, fill=red!20] (rl) at (0, -0.5) {RL Agent (Q-Learning)\\Look up $\mathbf{Q}(s_t, \cdot)$, Select $a_t^*$};
...
\node[box, fill=orange!20] (tx) at (0, -3) {Transmission Execution\\Aggregate, Encode, Transmit via WiFi Link};

% Arrow from RL to TX
\draw[arrow] (rl) -- (tx);

% Feedback loop
\node[box, fill=pink!20] (reward) at (4, -1.5) {Reward Computation\\(Multi-objective)};
\draw[arrow] (tx) -- (5, -2.2) -- (reward);

% Update Q-table (dotted arrow back)
\draw[arrow, dashed] (reward) -- (5, 1) -- (0.5, 1) -- (rl) node[midway, right, font=\small] {Update $\mathbf{Q}$ (offline)};

% Wireless channel
\node[box, fill=gray!20] (channel) at (0, -5) {Wireless Channel\\(Variable SNR, Loss, Stability)};
\draw[arrow] (tx) -- (channel);

% Receiver
\node[box, fill=gray!20] (rx) at (0, -7) {Receiver (GCS / UAV)\\Packet Arrives or Drops};
